...
<main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-300">
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="p-6 bg-white rounded-md shadow-md">
            <h2 class="text-lg text-gray-700 font-semibold capitalize">TAMBAH MAKANAN</h2>
            <hr class="mt-4">
            <form action="{{ route('admin.foods.update', $food->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 gap-6 mt-4">
                    ...
                    <div>
                        <label class="text-gray-700" for="name">JUDUL MAKANAN</label>
                        <input class="form-input w-full mt-2 rounded-md bg-gray-200 focus:bg-white" type="text" name="title" value="{{ old('title',$food->title) }}" placeholder="Judul Makanan" value="{{ $food->title }}">
                        @error('title')
                            <div class="w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
                                <div class="px-4 py-2">
                                    <p class="text-gray-600 text-sm">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select class="w-full border bg-gray-200 focus:bg-white rounded px-3 py-2 outline-none" name="category_id">
                            @foreach($categories as $category)
                                <option class="py-1" value="{{ $category->id }}" @if ($food->category_id == $category->id) selected
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="5">{{ old('description',$food->description) }}</textarea>
                        @error('description')
                            <div class="w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
                                <div class="px-4 py-2">
                                    <p class="text-gray-600 text-sm">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input class="form-input w-full mt-2 rounded-md bg-gray-200 focus:bg-white p-3" type="number" name="price" value="{{ old('price',$food->price) }}" placeholder="Harga, Ex: 10000000">
                        @error('price')
                            <div class="w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
                                <div class="px-4 py-2">
                                    <p class="text-gray-600 text-sm">{{ $message }}</p>
