@extends('layouts.app', ['title' => 'Foods - Admin'])
    ...
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="flex items-center">
            <button class="text-white focus:outline-none bg-gray-600 px-4 py-2 shadow-sm rounded-md">
                <a href="{{ route('admin.foods.create') }}">TAMBAH</a>
            </button>
                    ...
                    <thead class="justify-between">
                        <tr class="bg-gray-600 w-full">
                            <th class="px-16 py-2" style="width: 40%">
                                <span class="text-white">JUDUL MAKANAN</span>
                            </th>
                    ...
                    <tbody class="bg-gray-200">
                        @forelse($foods as $food)
                            <tr class="border bg-white">
                                ...
                                <td class="px-16 py-2">
                                    {{ moneyFormat($food->price) }}
                                </td>
                                <td class="px-10 py-2 text-center">
                                    <a href="{{ route('admin.foods.edit', $food->id) }}" class="bg-indigo-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">EDIT</a>
                                    <button onClick="destroy(this.id)" id="{{ $food->id }}" class="bg-red-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">HAPUS</button>
                                </td>
                            </tr>
                        @empty
